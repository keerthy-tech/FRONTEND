...
export class DashboardComponent implements OnInit {
  ...
  getTasks(): void {
    ...
    this.http.get<any>('https://cyber2-be.onrender.com/api/todos/gettask', { headers }).subscribe(
      response => {
        console.log(response);
  ...
  deleteTask(taskId: string): void {
    ...
    this.http.delete<any>(`https://cyber2-be.onrender.com/api/todos/deletetask/${taskId}`, { headers }).subscribe(
      response => {
        console.log(response);
  ...
  markAsComplete(taskId: string): void {
    ...
    this.http.patch<any>(`https://cyber2-be.onrender.com/api/todos/edittask/${taskId}`, { status: true }, { headers }).subscribe(
      response => {
        console.log(response);
        alert(response.message)
  ...
  markAsImportant(taskId: string): void {
    ...
    this.http.patch<any>(`https://cyber2-be.onrender.com/api/todos/edittask/${taskId}`, { important: true }, { headers }).subscribe(
      response => {
        console.log(response);
        alert(response.message)
